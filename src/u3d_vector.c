...
void u3d_vectorCopyFromRawData(U3DVector_ptr dst, number src_raw_data[4]){
...
void u3d_vectorCrossProduct(U3DVector_ptr left, U3DVector_ptr right){
	number x = left->y * right->z - left->z * right->y;
	number y = left->z * right->x - left->x * right->z;
	number z = left->x * right->y - left->y * right->x;
	left->x = x;
	left->y = y;
	left->z = z;
	left->w = 0.0;
...
void u3d_vectorNormalize(U3DVector_ptr target){
	number len = u3d_vectorLength(target);
	target->x /= len;
	target->y /= len;
	target->z /= len;
}
